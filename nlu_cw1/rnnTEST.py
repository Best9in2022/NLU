...
class RNNTEST(Model):
	...
	def predict(self, x):
		...
		for t in range(len(x)):
			...
			s[t] = relu(self.V@x_t+ self.U@s[t-1])
	...
	def acc_deltas_np(self, x, d, y, s):
		...
		deltaOut = d_t - y[t]  #(v,1)
		self.deltaW+= np.outer(deltaOut, s[t]) #(v,h)
		deltaIn = self.W.T@deltaOut*grad1(s[t]) #(h,v)@(v,1)*(h,1)
		x_t = make_onehot(x[t], self.vocab_size) 
		self.deltaV+= np.outer(deltaIn, x_t)   #(h,v)
		self.deltaU+= np.outer(deltaIn, s[t-1]) #(h,h)
		##########################
		...
		for t in reversed(range(len(x))):
			...
			for T in range(1,steps+1):
				if t-T-1>=-1: 
					...
					deltaIn_T = self.U.T@deltaIn*grad1(s[t-T])   # U * upper_layer_acti * current_layer_acti 
