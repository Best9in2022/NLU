...
class LSTMEncoder(Seq2SeqEncoder):
    ...
    def __init__(self,
                 ...
                 num_layers=1,
                 bidirectional=True,
                 dropout_in=0.25,   #dropout in input
                 dropout_out=0.25,
    ...
    def forward(self, src_tokens, src_lengths):
        ...
        if self.bidirectional:
            def combine_directions(outs):
                return torch.cat([outs[0: outs.size(0): 2], outs[1: outs.size(0): 2]], dim=2)   #turn from alternative to continuous
...
class AttentionLayer(nn.Module):
    ...
    def forward(self, tgt_input, encoder_out, src_mask):
        ...
        encoder_out = encoder_out.transpose(1, 0)   # [batch_size, src_time_steps, output_dims]

        attn_scores = self.score(tgt_input, encoder_out)  # [batch_size, 1, src_time_steps]
        ...
        attn_weights = F.softmax(attn_scores, dim=-1) # attn_weights.size = [batch, 1, src_time_steps]
        attn_context = torch.bmm(attn_weights, encoder_out).squeeze(dim=1) # attn_context.size = [batch, output_dims]

        context_plus_hidden = torch.cat([tgt_input, attn_context], dim=1) # context_plus_hidden.size = [batch, intput_dims + output_dims]
        attn_out = torch.tanh(self.context_plus_hidden_projection(context_plus_hidden)) # attn_out.size = [batch, output_dims]
...
class LSTMDecoder(Seq2SeqDecoder):
    ...
    def __init__(self,
        ...
        if self.use_lexical_model:
            ...
            # TODO: --------------------------------------------------------------------- CUT
            self.lexical_model = nn.Linear(embed_dim, embed_dim)
    ...
    def forward(self, tgt_inputs, encoder_out, incremental_state=None):
        ...
        src_out, src_hidden_states, src_cell_states = encoder_out['src_out']     # src_out:(src_seq_len, batch_size, hidden_size)
        ...
        for j in range(tgt_time_steps):
            ...
            lstm_input = torch.cat([tgt_embeddings[j, :, :], input_feed], dim=1)   #[1, n, d + h]
            ...
            ---------------A: Because previous target state is used to attend to the hidden states from the encoder to obtain informatin from the encoder output (conditional language model),
                              then get the output for this time step
            ...
            if self.attention is None:
                ...
                input_feed, step_attn_weights = self.attention(tgt_hidden_states[-1], src_out, src_mask) #(n, h); (n, t_in)
                attn_weights[:, j, :] = step_attn_weights   #(n, t_in)
                ...
                if self.use_lexical_model:
                    ...
                    scr_context = torch.bmm(step_attn_weights.unsqueeze(1), src_embeddings_copy) # [n, 1, d]
                    scr_context = nn.tanh(scr_context.unsqueeze(1)) # [n, d]
        ...
        # Final projection
        decoder_output = self.final_projection(decoder_output)  # [n, tgt_time_steps, V]

        if self.use_lexical_model:
            # __QUESTION-4: Incorporate the LEXICAL MODEL into the prediction of target tokens here
            # TODO: --------------------------------------------------------------------- CUT
            # transfer a list to tensor
            context_output = torch.cat(lexical_contexts, dim=0).view(tgt_time_steps, batch_size, self.embed_dim)
