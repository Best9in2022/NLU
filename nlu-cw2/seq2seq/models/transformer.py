...
class TransformerEncoder(Seq2SeqEncoder):
    ...
    def forward(self, src_tokens, src_lengths):
        ...
        # shape: [batch_size, max_source_positions, encoder_embed_dim]
        # embeddings: [batch_size, src_time_steps, encoder_embed_dim]

        # Clone for output state
        src_embeddings = embeddings.clone()
        # src_embeddings: [batch_size, src_time_steps, encoder_embed_dim]
        ...
        We need to model the word order. 
        '''
        ...
        # forward_state: [batch_size, src_time_steps, encoder_embed_dim]
        ...
        # shape: [max_source_positions, batch_size, encoder_embed_dim]
        # forward_state: [src_time_steps, batch_size, encoder_embed_dim]

        # Compute padding mask for attention
        encoder_padding_mask = src_tokens.eq(self.padding_idx) # True if position index is padding_idx
...
class TransformerDecoder(Seq2SeqDecoder):
    ...
    def forward(self, tgt_inputs, encoder_out=None, incremental_state=None, features_only=False):
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            We use decoder to predict the next word, when computing the current output, the self attention layer 
            in the decoder should only use the information of the current word and all the words before it, it 
            cannot use the information of the words after the current word, so we need to use self_attn_mask. 
            3.  Why do we need it in the decoder but not in the encoder?
            Because we use decoder to predict the next word, so we need self_attn_mask, in the encoder, we need 
            to use the information from all the inputs to fully represent the input sequences, these features can 
            then be used by the decoder through cross-attention, so we do not need self_attn_mask in the encoder. 
            4.  Why do we not need a mask for incremental decoding?
            Because in the incremental decoding, the input to the decoder is the single previous token, the input 
        ...
        if not features_only:
            ...
            We use a linear projection to project each output vector from output embedding size to vocabulary size
            so that  we can use softmax to normalize it to form a distribution over the target vocabulary size 
            and generate the words. 
            2.  What would the output represent if features_only=True?
            If feature_only=True, the output can represent the feature vector of the target words at each time step, 
            in other words, it can be seen as the word embeddings for the target words. 
            '''
